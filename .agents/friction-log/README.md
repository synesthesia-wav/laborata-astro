...
```sh
bunx frog list    # what is already known
bunx frog log     # add one
```

`bunx frog log` writes the sections to fill in. Each id is when the friction was hit plus its title, so
